...
ax1.set_xlabel('Execution Time (Seconds)', fontsize=11, fontweight='bold', color='#475569')
...
for bar, time in zip(bars1, times):
    ...
    ax1.text(width * 1.05, bar.get_y() + bar.get_height()/2.0, label, 
...
ax2.set_xlabel('Speedup vs. Sequential', fontsize=11, fontweight='bold', color='#475569')
...
for bar, speedup in zip(bars2, speedups):
    ...
    ax2.text(width * 1.05, bar.get_y() + bar.get_height()/2.0, label, 
             ha='left', va='center', fontsize=11, fontweight='bold', color='#0F172A')
...
ax.set_xlabel('Execution Time (Seconds)', fontsize=11, fontweight='bold', color='#475569')
...
for bar, time in zip(bars, times):
    width = bar.get_width()
    label = f'{time:.2f} s' if time >= 1 else f'{time:.3f} s'
    ax.text(width * 1.05, bar.get_y() + bar.get_height()/2.0, label, 
            ha='left', va='center', fontsize=11, fontweight='bold', color='#0F172A')
...
ax.set_xlabel('Speedup Factor (x)', fontsize=11, fontweight='bold', color='#475569')
ax.grid(True, axis='x', linestyle='--', color='#E2E8F0', alpha=0.9)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
ax.spines['left'].set_visible(False)
